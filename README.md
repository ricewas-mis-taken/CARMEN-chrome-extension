...
Hard lock never rewrites the offending tab's own URL and never opens a whitelisted domain in a new tab — it only ever changes which tab is *focused*. If a whitelisted tab is already open and visible, it switches to it, leaving the offending tab exactly where it was, just unfocused; otherwise it opens a new tab to the browser's own homepage/new-tab page, same treatment. Two consequences:

- **No more fallback-redirect loops**: an earlier version opened a new tab to the first whitelist entry when nothing else was available. If that entry was a redirect-prone URL (a marketing/landing page rather than the actual app), the redirect would land somewhere no longer whitelisted, reading as a fresh violation and reopening the same URL again — forever. Opening the homepage instead of a whitelist URL doesn't have this problem, since the homepage can never itself be a violation.
